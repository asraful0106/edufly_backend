...
// For upload student Image and Signature (temp storage; we move later)
export const uploadStudentFiles = multer({
    storage: multer.diskStorage({}),
    fileFilter: (req, file, cb) => {
        const allowedTypes = ['image/jpeg', 'image/png', 'image/gif', 'image/avif'];
        if (allowedTypes.includes(file.mimetype)) cb(null, true);
        else cb(new Error('Only images and gif are allowed!'), false);
    }
}).fields([
    { name: 'image', maxCount: 1 },
    { name: 'signature', maxCount: 1 }
...
const createStudent = async (req, res) => {
    ...
    const isBatchExist = await prisma.batches.findUnique({ where: { id: batch_id } });
    if (!isBatchExist) return res.status(400).json({ message: "Batch is not found!" });

    try {
        const {
            name_eng, name_bng, student_id,
            class_roll, email, phone_number,
            religion, gender, date_of_birth,
            present_adress, parmanent_adress,
            position, blood_group, role
        } = req.body;

        if (!name_eng || !name_bng || !student_id || !email || !phone_number || !date_of_birth || !req.files?.image) {
            return res.status(400).json({
                status: 'error',
                message: 'Missing required fields or profile iamge.'
            });
        ...
        if (req.files.image) {
            ...
            const fileName = `${req.files.image[0].filename}${ext}`;
            await fse.move(tempPath, path.join(studentImageUplodDirName, fileName));
        }
        if (req.files.signature) {
            const tempPath = req.files.signature[0].path;
            const ext = path.extname(req.files.signature[0].originalname);
            const fileName = `${req.files.signature[0].filename}${ext}`;
            await fse.move(tempPath, path.join(studentSignetureUplodDirName, fileName));
        }
...
// Hard delete (optional; not used by ManageStudent clear placement)
const deleteStudent = async (req, res) => {
    try {
        const { student_id } = req.params;
        const { institution_id } = req.body; // or req.query
        ...
        await prisma.students.deleteMany({
            where: { id: student_id, institution_id }
        });
...
// Get all student (returns related IDs needed by UI)
const getAllStudent = async (req, res) => {
    try {
        const institution_id = req.params.institution_id;
        if (!institution_id) {
...
// Update only placement (batch/class/section) — allows clearing class/section
const updatePlacement = async (req, res) => {
    try {
        const { id } = req.params;
        const student = await prisma.students.findUnique({ where: { id } });
        ...
        const reqBatchId = Object.prototype.hasOwnProperty.call(req.body, 'batch_id') ? req.body.batch_id : undefined;
        const reqClassId = Object.prototype.hasOwnProperty.call(req.body, 'class_id') ? req.body.class_id : undefined;
        const reqSectionId = Object.prototype.hasOwnProperty.call(req.body, 'section_id') ? req.body.section_id : undefined;

        // validate provided ids (non-null) belong to same institution
        const checks = [];
        if (reqBatchId !== undefined && reqBatchId !== null) {
            checks.push(prisma.batches.findFirst({ where: { id: reqBatchId, institution_id: student.institution_id } }));
        }
        if (reqClassId !== undefined && reqClassId !== null) {
            checks.push(prisma.classes.findFirst({ where: { id: reqClassId, institution_id: student.institution_id } }));
        }
        if (reqSectionId !== undefined && reqSectionId !== null) {
            checks.push(prisma.sections.findFirst({
                where: { id: reqSectionId, class: { institution_id: student.institution_id } }
            }));
        }
        const found = await Promise.all(checks);
        if (found.some((x) => x === null)) {
            return res.status(400).json({ message: "Invalid batch/class/section for this institution." });
        }

        const data = {};
        // batch_id cannot be null per schema; ignore attempt to null
        if (reqBatchId !== undefined) {
            if (reqBatchId === null) {
                return res.status(400).json({ message: "batch_id cannot be null (schema restriction)." });
            }
            data.batch_id = reqBatchId;
            // if batch changed and class/section not explicitly provided, clear them
            if (reqClassId === undefined) data.class_id = null;
            if (reqSectionId === undefined) data.section_id = null;
        }
        if (reqClassId !== undefined) data.class_id = reqClassId;     // may be null
        if (reqSectionId !== undefined) data.section_id = reqSectionId; // may be null
...
// General update (edit drawer) — does NOT touch placement fields
